...
def printResult(self,SWTrans,SWOrien):
    title='#centreNo'+' '+'#SWTrans'+' '+'#SWOrien'+' '+'#SWTotal'+'\n'
    self.write(title)
    for CentreNo in range(len(SWTrans)):
        line=str(CentreNo)+'    '+str(SWTrans[CentreNo])+'  '+str(SWOrien[CentreNo])+'  '+str(SWTrans[CentreNo]+SWOrien[CentreNo])+'\n'
        self.write(line)
    self.close()
...
printResult(outputFile,SWTrans,SWOrien)
